...
// =============================================================================
// STARS GENESIS RENDERER
// =============================================================================

// Helper for deep merging animation config
function mergeAnimationConfig(
  defaults: StarsGenesisAnimationConfig,
  overrides?: Partial<StarsGenesisAnimationConfig>
): StarsGenesisAnimationConfig {
...
export class StarsGenesis {
  ...
  private physics: StarsGenesisPhysicsConfig
  private visual: StarsGenesisVisualConfig
  private formation: StarsGenesisFormationConfig
  private animation: StarsGenesisAnimationConfig
  private audioEffects: StarsGenesisAudioEffects
  ...
  constructor(options: StarsGenesisOptions) {
    ...
    // Merge configs with defaults
    this.starCount = options.starCount ?? defaultStarsGenesisConfig.starCount
    this.physics = { ...defaultStarsGenesisPhysics, ...options.physics }
    this.visual = { ...defaultStarsGenesisVisual, ...options.visual }
    this.formation = { ...defaultStarsGenesisFormation, ...options.formation }
    this.animation = mergeAnimationConfig(defaultStarsGenesisAnimation, options.animation)
    this.audioEffects = { ...defaultStarsGenesisAudioEffects, ...options.audioEffects }
    this.scale = options.scale ?? defaultStarsGenesisConfig.scale
    this.transitionDuration = options.transitionDuration ?? defaultStarsGenesisConfig.transitionDuration

    // Create container group
    this.group = new THREE.Group()
    this.group.name = 'stars-genesis-avatar'
    ...
    logger.info('StarsGenesis renderer initialized', { starCount: this.starCount })
  }
  ...
  private generateFormationPosition(radius: number, index: number): THREE.Vector3 {
    ...
    switch (this.formation.type) {
      case 'sphere': {
        ...
        const phi = Math.acos(1 - 2 * (index + 0.5) / this.starCount)
      ...
      case 'ring': {
        const angle = (index / this.starCount) * Math.PI * 2
  ...
  private createGeometry(): void {
    ...
    const delays = new Float32Array(this.starCount)
  ...
  private createMaterial(): void {
    ...
    this.material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uBaseSize: { value: this.visual.starSize * 50 },  // Adjusted multiplier
        uColor: { value: color },
  ...
  private updateStars(delta: number): void {
    ...
    for (let i = 0; i < this.starCount; i++) {
      ...
      if (this.isExploding) {
        // Explosion: stars fly away from explosion center
  ...
  private handleClick(event: MouseEvent): void {
    ...
    // Check if click is near the star formation
  ...
  private triggerExplosion(x: number, y: number): void {
    ...
    logger.debug('Stars explosion triggered')
  }
  ...
  public startListening(): void {
    ...
    logger.debug('StarsGenesis: listening started')
  }

  public stopListening(): void {
    this.isFollowing = false
    logger.debug('StarsGenesis: listening stopped')
  }

  public startThinking(): void {
    // Increase leader movement speed while thinking
    this.physics.leaderSpeed = defaultStarsGenesisPhysics.leaderSpeed * 2.5
    this.isFollowing = true
    logger.debug('StarsGenesis: thinking started')
  }

  public stopThinking(): void {
    this.physics.leaderSpeed = defaultStarsGenesisPhysics.leaderSpeed
    this.isFollowing = false
    logger.debug('StarsGenesis: thinking stopped')
  }
  ...
  public setStarSize(size: number): void {
    this.visual.starSize = size
  ...
  public setPhysics(physics: Partial<StarsGenesisPhysicsConfig>): void {
    this.physics = { ...this.physics, ...physics }
  }

  public setAnimation(animation: Partial<StarsGenesisAnimationConfig>): void {
    this.animation = mergeAnimationConfig(this.animation, animation)
  }

  public setAudioEffects(effects: Partial<StarsGenesisAudioEffects>): void {
  ...
  public setRandomStars(): void {
    ...
    // Randomize star size
    this.setStarSize(0.4 + Math.random() * 0.8)

    logger.debug('Stars randomized')
  }

  public updateGeometryFromStars(): void {
   ...
   */
  public getVisualConfig(): StarsGenesisVisualConfig {
    return { ...this.visual }
  }
   ...
   */
  public getPhysicsConfig(): StarsGenesisPhysicsConfig {
    return { ...this.physics }
  }
   ...
   */
  public getAnimationConfig(): StarsGenesisAnimationConfig {
    return JSON.parse(JSON.stringify(this.animation))
  }

  /**
   * Get current audio effects config
   */
  public getAudioEffectsConfig(): StarsGenesisAudioEffects {
   ...
   */
  public getFormationConfig(): StarsGenesisFormationConfig {
  ...
  public exportGLTF(): void {
    ...
    exporter.parse(
      ...
      (gltf) => {
        ...
        link.href = url
        link.download = 'stars-genesis-avatar.gltf'
        link.click()
  ...
  public dispose(): void {
    ...
    logger.info('Stars Genesis renderer disposed')
  }
}
