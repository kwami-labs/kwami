<template>
  <div class="nft-verification">
    ...
    <div v-else class="verified">
      <UIcon name="i-heroicons-check-circle" class="icon success" />
      ...
      <div v-if="nftStore.kwamiCount > 1" class="select-nft">
        <USelect
          ...
          :options="nftOptions"
          placeholder="Select KWAMI NFT"
          @update:model-value="handleNFTSelect"
        />
      </div>
    </div>
  </div>
...
const handleNFTSelect = (mint: string) => {
  const nft = nftStore.kwamiNFTs.find(n => n.mint === mint);
  if (nft) {
