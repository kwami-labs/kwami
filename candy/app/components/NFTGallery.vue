<template>
  <div class="nft-gallery">
    <div v-if="nftStore.loading" class="text-center py-12">
      ...
      <p class="text-gray-400">Loading your KWAMIs...</p>
    </div>

    <div v-else-if="nftStore.ownedNfts.length === 0" class="text-center py-12">
      <div class="w-24 h-24 bg-gray-900 rounded-full flex items-center justify-center mx-auto mb-4">
        <UIcon name="i-heroicons-cube-transparent" class="w-12 h-12 text-gray-600" />
      </div>
      <p class="text-xl text-gray-400 mb-2">No KWAMIs Yet</p>
      ...
      >
        <template #header>
          <div class="aspect-square bg-gray-900 rounded-lg overflow-hidden">
            ...
            <div v-else class="w-full h-full flex items-center justify-center">
              <UIcon name="i-heroicons-cube-transparent" class="w-16 h-16 text-gray-600" />
            </div>
          </div>
        </template>
        ...
        <div class="space-y-3">
          ...
          <div class="p-3 bg-gray-900 rounded-lg">
            <p class="text-xs text-gray-400 mb-1">DNA</p>
            <p class="font-mono text-xs break-all text-gray-300">{{ nft.dna.slice(0, 32) }}...</p>
          </div>
