...
```
qwami-token/target/idl/qwami_token.json   # QWAMI Token IDL
kwami-nft/target/idl/kwami_nft.json       # KWAMI NFT IDL
```
...
cp qwami-token/target/idl/qwami_token.json ../../../candy/src/types/
cp kwami-nft/target/idl/kwami_nft.json ../../../candy/src/types/
```
...
│   └── mainnet-latest.json
├── qwami-token/                                     # QWAMI token workspace + addresses (committed)
│   ├── localnet-addresses.json
│   ├── devnet-addresses.json
│   ├── testnet-addresses.json
│   └── mainnet-addresses.json
└── kwami-nft/                                       # KWAMI NFT workspace + addresses (committed)
    ├── localnet-addresses.json
    ├── devnet-addresses.json
    ├── testnet-addresses.json
    └── mainnet-addresses.json
...
cat qwami-token/devnet-addresses.json | jq
cat kwami-nft/testnet-addresses.json | jq

# Get specific values
jq -r '.programId' qwami-token/devnet-addresses.json
jq -r '.collectionMint' kwami-nft/devnet-addresses.json

# Compare across clusters
echo "Localnet: $(jq -r '.programId' qwami-token/localnet-addresses.json 2>/dev/null || echo 'Not deployed')"
echo "Devnet:   $(jq -r '.programId' qwami-token/devnet-addresses.json 2>/dev/null || echo 'Not deployed')"
echo "Testnet:  $(jq -r '.programId' qwami-token/testnet-addresses.json 2>/dev/null || echo 'Not deployed')"
echo "Mainnet:  $(jq -r '.programId' qwami-token/mainnet-addresses.json 2>/dev/null || echo 'Not deployed')"
```
...
- `qwami-token/{cluster}-addresses.json` - Canonical deployment state
- `kwami-nft/{cluster}-addresses.json` - Canonical deployment state
...
for cluster in localnet devnet testnet mainnet; do
  echo "=== $cluster ==="
  jq -r '.programId' qwami-token/${cluster}-addresses.json 2>/dev/null || echo "Not deployed"
done
...
CLUSTER="devnet"
PROGRAM_ID=$(jq -r '.programId' qwami-token/${CLUSTER}-addresses.json)
echo "https://explorer.solana.com/address/${PROGRAM_ID}?cluster=${CLUSTER}"

# Open collection mint on explorer
COLLECTION=$(jq -r '.collectionMint' kwami-nft/devnet-addresses.json)
...
│ • Fixed $0.01 price │ • Dynamic pricing (by generation) │
└─────────────────────┴───────────────────────────────────┘
                ┌─────────────────────────┐
                │        KWAMI DAO         │
                │       (Governance)       │
                ...
                │ • Proposals + voting     │
                │ • Quorum + delays        │
                │ • Uses QWAMI for voting  │
                └─────────────────────────┘
```

### 1. QWAMI Token Program

**Location**: `qwami-token/programs/`  
...
- **QWAMI Treasury PDA**: `["qwami-treasury"]`
  - Tracks total SOL received from QWAMI buys
  - Tracks mint counts (SOL-only MVP)
...
- ✅ **SOL Payment**: All mints require SOL
- ✅ **Linear Pricing**: 0.1 SOL in 2026, 0.2 SOL in 2027, 0.3 SOL in 2028, etc

#### Generation Pricing (SOL)

- **Gen #0 (2026)**: 0.1 SOL
- **Gen #1 (2027)**: 0.2 SOL
- **Gen #2 (2028)**: 0.3 SOL
- … continues linearly (+0.1 SOL per generation)
...
// Mint a unique KWAMI NFT (requires SOL payment)
mint_kwami(
    dna_hash: [u8; 32],
...
// Burn NFT (no SOL refund in SOL-paid MVP)
burn_kwami()
...
- ✅ Proposals lifecycle: create, vote, finalize, cancel
- ✅ Configurable quorum, voting periods, execution delays
- ✅ Uses QWAMI token accounts for stake-weighted voting
...
- **KwamiTreasury PDA**: `["kwami-treasury"]`
  - Tracks total SOL received (accounting only)
  - Tracks mint/burn counts
...
Exchange:      SOL ↔ QWAMI (SOL-only MVP; add USDC/others later)
```
...
│   ├── devnet-addresses.json          # Deployed addresses (also <cluster>-addresses.json)
│   ├── Anchor.toml
│   ├── Cargo.toml
│   ├── package.json
│   └── tsconfig.json
...
│   └── tsconfig.json
│
├── kwami-dao/                           # KWAMI DAO Program
│   ├── programs/
│   │   ├── src/lib.rs                 # DAO logic
│   │   └── Cargo.toml
│   ├── scripts/
│   │   └── initialize-dao.ts          # Initialization script
│   ├── target/
│   │   ├── idl/kwami_dao.json         # Interface definition
│   │   └── types/kwami_dao.ts         # TypeScript types
│   ├── devnet-addresses.json          # Deployed addresses (also <cluster>-addresses.json)
│   ├── Anchor.toml
│   ├── Cargo.toml
│   ├── package.json
...
│
├── scripts/deploy-programs.sh          # Unified deployment script
├── scripts/initialize-programs.sh      # Initialization script
├── scripts/install-dependencies.sh     # Installs Solana/Anchor/Rust
...
1. **Unit Tests** (Run with `anchor test`)
   - `qwami-token/tests/qwami-token.ts` - Token operations
   - `qwami-token/tests/qwami-token-economic.ts` - Economic features
   - `kwami-nft/tests/kwami-nft-economic.ts` - NFT economic features
...
cd qwami-token && npm install
cd ../kwami-nft && npm install
cd ../kwami-dao && npm install

# Run unit tests
cd qwami-token && anchor test
cd ../kwami-nft && anchor test
cd ../kwami-dao && anchor test

# Run integration tests
cd .. && anchor test

# Test on devnet (after deployment)
cd qwami-token && ANCHOR_PROVIDER_URL=https://api.devnet.solana.com \
  ANCHOR_WALLET=$HOME/.config/solana/id.json \
...
- **`qwami-token/scripts/initialize-qwami.ts`**:
  - Creates QWAMI mint
  - Creates USDC vault
  - Initializes token authority PDA
  - Initializes treasury PDA

- **`kwami-nft/scripts/initialize-kwami.ts`**:
