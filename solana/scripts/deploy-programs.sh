...
# Check expected workspaces exist (folder structure was refactored)
if [ ! -d "$QWAMI_DIR" ] || [ ! -d "$KWAMI_DIR" ]; then
    echo -e "${RED}❌ Error: Expected Anchor workspaces not found.${NC}"
...
QWAMI_LIB_RS="qwami-token/programs/src/lib.rs"
...
KWAMI_LIB_RS="kwami-nft/programs/src/lib.rs"
...
if ! command -v node &> /dev/null || ! command -v npm &> /dev/null; then
    ...
    echo -e "   cd qwami-token && npm run initialize"
    echo -e "   cd kwami-nft && npm run initialize"
    INIT_SKIPPED=true
else
    # Initialize QWAMI
...
echo -e "QWAMI details: ${BLUE}qwami-token/${CLUSTER}-addresses.json${NC}"
echo -e "KWAMI details: ${BLUE}kwami-nft/${CLUSTER}-addresses.json${NC}"
echo -e "\n${YELLOW}📄 IDL Files:${NC}"
echo -e "QWAMI IDL: ${BLUE}qwami-token/target/idl/qwami_token.json${NC}"
echo -e "KWAMI IDL: ${BLUE}kwami-nft/target/idl/kwami_nft.json${NC}"
...
if [ "$INIT_SKIPPED" = true ] || [ "$QWAMI_INITIALIZED" = false ] || [ "$KWAMI_INITIALIZED" = false ]; then
    ...
    [ "$QWAMI_INITIALIZED" = false ] && echo -e "2. Run: cd qwami-token && npm run initialize"
    [ "$KWAMI_INITIALIZED" = false ] && echo -e "3. Run: cd kwami-nft && npm run initialize"
