...
/// Conversion rate used by the QWAMI token program:
/// 1 QWAMI = 1_000_000 lamports (i.e. 1 SOL = 1,000 QWAMI)
const LAMPORTS_PER_QWAMI: u64 = 1_000_000;
...
/// Convert QWAMI-denominated costs to lamports using the fixed conversion rate.
fn qwami_to_lamports(amount_qwami: u64) -> Result<u64> {
    amount_qwami
        .checked_mul(LAMPORTS_PER_QWAMI)
...
pub mod kwami_nft {
    ...
    pub fn mint_kwami(
        ...
        let (base_cost_qwami, platform_fee_qwami, transaction_fee_qwami, total_cost_qwami) =
            calculate_mint_cost(current_generation);

        // Convert to lamports for SOL-only payment model
        let base_cost_lamports = qwami_to_lamports(base_cost_qwami)?;
        let total_cost_lamports = qwami_to_lamports(total_cost_qwami)?;
        ...
        // Transfer SOL (lamports) from user to treasury PDA (program-owned account can hold lamports)
        system_transfer(
            CpiContext::new(
        ...
        let dividend_amount = (total_cost_lamports * DIVIDEND_POOL_PERCENTAGE) / 100;
        let operations_amount = (total_cost_lamports * OPERATIONS_FUND_PERCENTAGE) / 100;
        treasury.dividend_pool_balance = treasury
        ...
        msg!(
            "Cost: {} lamports (Base: {} | Platform Fee: {} QWAMI | TX Fee: {} QWAMI | Total: {} QWAMI)",
            total_cost_lamports,
            base_cost_lamports,
            platform_fee_qwami,
            transaction_fee_qwami,
            total_cost_qwami
        );
        ...
        msg!(
            "Treasury Revenue: {} lamports (Dividends: {} | Operations: {})",
            total_cost_lamports,
            dividend_amount,
            operations_amount
        );
    ...
    /// Burn Kwami NFT and remove DNA from registry (with 50% SOL refund)
    ...
    pub fn burn_kwami(ctx: Context<BurnKwami>) -> Result<()> {
        ...
        // Calculate refund (50% of original base cost in lamports)
        let refund_amount = calculate_burn_refund(kwami_nft.mint_cost_lamports);

        // Validate treasury has sufficient SOL for refund (must remain rent-exempt)
        ...
        // Transfer lamports by directly adjusting balances (treasury is program-owned)
        ...
        **ctx.accounts.owner.to_account_info().try_borrow_mut_lamports()? = ctx
        ...
        // Update treasury accounting
        treasury.total_sol_refunded = treasury
            .total_sol_refunded
            .checked_add(refund_amount)
            .ok_or(ErrorCode::MathOverflow)?;
        ...
        msg!("DNA Hash: {:?}", dna_hash);
        msg!(
            "SOL Refund: {} lamports (50% of {} base cost)",
            refund_amount,
            kwami_nft.mint_cost_lamports
        );
        msg!("Freed DNA for re-minting");
        msg!("Treasury Refunded: {} lamports", refund_amount);
...
pub struct KwamiTreasury {
    ...
    /// Total SOL (lamports) refunded from NFT burns
    pub total_sol_refunded: u64,
