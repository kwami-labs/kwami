...
export async function createKwamiCollection(
  ...
  try {
    ...
    // Create the collection mint off-chain (the on-chain initialize expects it to exist
    // and have mint_authority = collectionAuthority + decimals = 0)
    console.log('🏗️  Creating collection mint off-chain...');
    await createMint(
      connection,
      wallet,
